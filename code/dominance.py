#!/usr/bin/env python3
"""— David's score + Bradley-Terry per layer; cross-layer Spearman + PCA. Direct test of h2."""
...
def main() -> None:
    ...
    ds_df.to_csv(TABLE_DIR / "dominance_davids_scores.csv")
    ...
    corr_df.to_csv(TABLE_DIR / "dominance_cross_layer_correlation.csv")
    ...
    save_figure(fig, "dominance", FIG_DIR)
    plt.close(fig)
...
Outputs:
- dominance_davids_scores.csv
- tableS3_dominance_concordance.csv
- dominance_cross_layer_correlation.csv
- dominance.png
"""
    replace_section(REPORT_PATH, "<!-- dominance RESULTS -->", "Directional influence, per layer and across layers", body)

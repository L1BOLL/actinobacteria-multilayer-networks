...
TABLES = REPO / "supplementary" / "tables"
...
def main() -> None:
    ...
    for i, a in enumerate(ORDER):
        for j, b in enumerate(ORDER):
            if i == j:
                ...
                ax.text(j, i, mark, ha="center", va="center", fontsize=9, color=INK, zorder=3)
    ...
    cb.set_label("Standardized overlap  $Z_{lm}$", fontsize=8)
    ...
    for i, r in enumerate(sig.itertuples()):
        ...
        ax.annotate(f"z = {r.z:+.1f}", (r.J_obs, i), textcoords="offset points",
                    ...
                    ha="right" if left else "left", fontsize=7.3, color=INK2)
    ax.set_yticks(y)
    ...
    for ext in FIGURE_FORMATS:
        fig.savefig(OUT / f"figure3_layer_overlap_null.{ext}", dpi=300, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"wrote figure3_layer_overlap_null.{{png,svg}}   ({n_over} over, {n_under} under, {n_draws:,} draws)")

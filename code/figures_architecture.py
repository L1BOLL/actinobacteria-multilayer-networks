...
from source_paths import EMBEDDING_CSV, FROZEN_ROOT, IDENTITY_CSV, RESULTS_DIR, find_layer_file

ROOT = FROZEN_ROOT
# Main-text figures land alongside the analysis figures, so results/figures/
# is the single place to look for anything the manuscript cites.
...
def build_ecological_identity(matrices: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    ecological_identity.to_csv(IDENTITY_CSV)
...
def make_figure_s1(matrices: dict[str, pd.DataFrame], stats_df: pd.DataFrame) -> None:
    ...
    for midx, (metric, title) in enumerate(zip(metrics, metric_titles)):
        ...
        ax.set_title(title)
    save_figure(fig, "figure_layer_architecture")
...
def make_figure_s3(matrices: dict[str, pd.DataFrame]) -> None:
    ...
    save_figure(fig, 'figure_raw_similarity')


def embedding_pca(df: pd.DataFrame) -> tuple[PCA, np.ndarray, np.ndarray]:
    """PCA of the 4-D embedding on **z-standardized** scores.

    The Methods specify "PCA was performed on z-standardized strain profiles".
    Running it on the raw embedding instead gives PC1 54.2% / PC2 23.3% rather
    than the 53.0% / 23.9% quoted in the text -- which is exactly the mismatch
    between the published figure axes and the manuscript body. Standardizing here
    keeps the figures and the text on the same computation.
    """
    X = StandardScaler().fit_transform(df[["DA", "IA", "MM", "MC"]].to_numpy(float))
    pca = PCA(n_components=2, random_state=0)
    coords = pca.fit_transform(X)
    loadings = pca.components_.T * np.sqrt(pca.explained_variance_)
    return pca, coords, loadings
...
def make_figure_s8(embeddings: pd.DataFrame, identity: pd.DataFrame) -> None:
    ...
    save_figure(fig, 'figure_embedding_archetypes')


def write_manifest(stats_df: pd.DataFrame) -> None:
    lines = ['Generated figures:', '  figure_layer_architecture.{png,svg}', '  figure_raw_similarity.{png,svg}', '  figure_role_stability.{png,svg}', '  figure_embedding_archetypes.{png,svg}', '', 'Layer statistics used in the figures:', stats_df.round(4).to_csv(index=True)]
...
def main() -> None:
    ...
    stats_df = build_stats_df(matrices)
    embeddings = pd.read_csv(EMBEDDING_CSV).rename(columns={'Unnamed: 0': 'strain'}) if EMBEDDING_CSV.exists() else None
    ...
    # drawn by code/figure_strategy_space.py, coloured by a continuous quantity.

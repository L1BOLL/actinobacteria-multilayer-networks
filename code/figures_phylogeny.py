...
Produces three figures, each as PNG (300 dpi) and PDF (vector, for typesetting):
  ...
  figure_phylogeny        main text  - ecological strategy is decoupled from 16S
  figure_metabolic_niche          main text  - assimilation does not predict network position
  figure_lambda_calibration supplement - why the asymptotic lambda test fails here
...
from data_io import FIGURE_FORMATS, save_figure, CATEGORY_MAP, FIG_DIR, LAYER_ORDER, SEED, canonical_strain, ensure_output_dirs, load_tensor
...
from metabolic_niche import load_profiles
...
def figure_h3(pgls: pd.DataFrame, mantel: pd.DataFrame, Dg: np.ndarray, Dp: np.ndarray) -> None:
    ...
    # The shaded "previously reported lambda = 0.31-0.48" band that stood here is
    # removed: it carried no citation anywhere in the manuscript, and this
    # analysis does not reproduce that range. A reference interval drawn without
    # a source invites the reader to compare against a number that is not
    # supported, which is the opposite of what this panel is for.
    ax.set_yticks(y)
    ...
    ax.text(0.97, 0.05, f"Mantel $\\rho$ = {rho:+.3f},  $P$ = {p:.2f}\n"
                        f"{N_MANTEL:,} strain-label permutations\n"
                        f"{len(x):,} dyads, not independent",
    ...
    ax.text(0.97, 0.93,
            ...
            f"{int((sizes > 1).sum())} groups of ≥2 hold {tied} strains ({tied / len(Dg):.0%})\n"
            f"{len(Dg) - n_geno} redundant tips",
...
def figure_metabolism() -> None:
    strain = pd.read_csv(FIG_DIR.parent / "tables" / "metabolic_niche_strain.csv")
    dyad = pd.read_csv(FIG_DIR.parent / "tables" / "metabolic_niche_dyad.csv")
    prof = pd.read_csv(FIG_DIR.parent / "tables" / "metabolic_niche_profiles.csv")
    ...
    save(fig, "figure_metabolic_niche")
...
def figure_calibration(pgls: pd.DataFrame, traits: pd.DataFrame, Dg: np.ndarray) -> None:
    ...
    save(fig, "figure_lambda_calibration")
...
def main() -> None:
    ...
    pgls = pd.read_csv(tables / "phylogenetic_signal.csv")
    mantel = pd.read_csv(tables / "mantel.csv")

#!/usr/bin/env python3
"""- Carbohydrate assimilation vs interaction network position.
...
def main() -> None:
    ...
    if not ATB_PATH.exists():
        replace_section(REPORT_PATH, "<!-- metabolic_niche RESULTS -->", "Metabolic niche and network position",
    ...
    strain_df.to_csv(TABLE_DIR / "metabolic_niche_strain.csv", index=False)
    ...
    dyad_df.to_csv(TABLE_DIR / "metabolic_niche_dyad.csv", index=False)
    ...
    profile_df.to_csv(TABLE_DIR / "metabolic_niche_profiles.csv", index=False)

    # Figure produced by figures_phylogeny.py (fig_metabolism).
...
Outputs: `metabolic_niche_strain.csv`, `metabolic_niche_dyad.csv`,
`metabolic_niche_profiles.csv`. Figure: `figures/fig_metabolism.*` (main text).
"""
    replace_section(REPORT_PATH, "<!-- metabolic_niche RESULTS -->", "Metabolic niche and network position", body,
    )

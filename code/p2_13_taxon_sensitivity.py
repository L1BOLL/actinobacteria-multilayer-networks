#!/usr/bin/env python3
"""P2.13 - Does anything depend on the four non-Streptomyces isolates?
...
from p2_2_gmm import cv_loglik
from p2_3_pca12 import parallel_analysis
from p2_4_jaccard_null import CONFOUND_PAIRS, jaccard
from p2_5_scc_null import largest_scc_size
...
N_DRAWS = 2000  # fewer than the primary 10,000: this is a robustness check
K_MAX = 8
...
def jaccard_null_baseline() -> tuple[int, int] | None:
    ...
    p2_4 already produces this with 10,000 draws and BY-FDR; regenerating a
...
def jaccard_null_summary(
    ...
    # Benjamini-Yekutieli, matching the primary analysis in p2_4.
...
def pca_summary(tensor: np.ndarray, seed: int) -> tuple[float, float, int]:
    ...
    null = parallel_analysis(X.shape[0], X.shape[1], 300, seed)
    return float(evr[0] * 100), float(evr[1] * 100), int((evr > null[: len(evr)]).sum())
...
def main() -> None:
    ...
    struct.to_csv(TABLE_DIR / "p2_13_layer_structure_n56.csv", index=False)

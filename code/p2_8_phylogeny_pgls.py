#!/usr/bin/env python3
"""P2.8 - Phylogenetic signal in interaction profiles. Tests h3.
...
def main() -> None:
    ...
    if not DIST_PATH.exists():
        replace_section(
            REPORT_PATH, "<!-- P2.8 RESULTS -->", "P2.8 Phylogenetic signal (PGLS)",
    ...
    diag_df.to_csv(TABLE_DIR / "p2_8_vcv_diagnostics.csv", index=False)

    all_lambda = pd.concat(
        [lambda_table(traits, C, order, label, rng, N_PERM) for label, C, order in topologies],
        ignore_index=True,
    )
    all_lambda.to_csv(TABLE_DIR / "p2_8_pgls.csv", index=False)

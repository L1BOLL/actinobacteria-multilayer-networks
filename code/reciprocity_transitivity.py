#!/usr/bin/env python3
"""— Reciprocity and transitivity against the degree-preserving null.
...
null model anywhere in the codebase; only the SCC did (connectivity). This stage supplies
both, reusing the cached draws so it costs nothing on top of layer_overlap/connectivity.
...
def transitivity(A: np.ndarray) -> float:
    ...
    manuscript names. ``figures_architecture.compute_layer_stats`` reports the mean
...
def reciprocity_pairs(A: np.ndarray) -> float:
    ...
    Same quantity as ``layer_descriptors.reciprocity_both``[0], which loops over all 1,770 dyads
...
def main() -> None:
    ...
    df.to_csv(TABLE_DIR / "reciprocity_transitivity.csv", index=False)

    # Three panels: SCC, reciprocity, transitivity. The SCC column is read from
    # connectivity's output rather than recomputed, so the two stages cannot disagree, and
    # all three degree-preserving comparisons appear in one figure.
    scc = pd.read_csv(TABLE_DIR / "connectivity_null.csv").set_index("layer")
    panels = [
    ...
    save_figure(fig, "figure_network_nulls", FIG_DIR)
    plt.close(fig)
...
Outputs:
- reciprocity_transitivity.csv
- figure_network_nulls.png
"""
    replace_section(REPORT_PATH, "<!-- reciprocity_transitivity RESULTS -->", "Reciprocity and transitivity against the null", body)

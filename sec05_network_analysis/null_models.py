...
def generate_degree_preserving_nulls(tensor: np.ndarray, n_draws: int = 1000, seed: int = SEED) -> dict[str, np.ndarray]:
    rng = np.random.default_rng(seed)
    out = {}
    for idx, layer in enumerate(LAYER_ORDER):
        A = tensor[idx].astype(np.uint8)
        layer_path = DATA_DIR / f"null_{layer}_{n_draws}.npy"
